function velChangeTimes = peakFilter(numPeaks, velChangeTimes, vel)
    ...
    for foo = 1:(numPeaks-2)
        ...
        % next one.
        if velChangeTimes(foo+1)-velChangeTimes(foo) < 100 &&...
                velChangeTimes(foo+2)-velChangeTimes(foo+1) < 100
            ...
            if mod(foo+1,2) == 0    % Local maximum
                % Flag smaller value
                if vel(velChangeTimes(foo)) < vel(velChangeTimes(foo+2))
