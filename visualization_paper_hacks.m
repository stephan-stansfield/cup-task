% visualization
%
% Plots experimental and simulated trials. Can plot individual trials and
% averages.
...
titleArray = {'IS', 'MM-FF', 'NI-FF', 'RB-FF', 'SM-FF', 'FM-FF', 'MM-NF',...
    'NI-NF', 'RB-NF', 'SM-NF', 'FM-NF','SB', 'SB-FF', 'SB-NF'};
parentFig = figure('Units','normalized','Position',[0,0,0.5,1]);
subplotFig = tiledlayout(parentFig,5,3);
...
% %         title(titleStr,'FontSize',titleFontSize)
%         title(titleArray{count},'FontSize',titleFontSize)
%         hold off;
        
        % Putting all simulation types into one subplot for a given subject
        figure(parentFig)
        nexttile(count);
        if plotExp
            plot(tNorm,velAvg,'Color','black','LineWidth',expWidth,'LineStyle',expStyle);
...
%         hold off;
        ...
        if save
            ...
            saveas(cp, strcat(saveFolder, cpStr, nameStr, pngSuffix))          
            saveas(bp, strcat(saveFolder, bpStr, nameStr, pngSuffix))                                                            
            saveas(cv, strcat(saveFolder, cvStr, nameStr, pngSuffix))
            saveas(bv, strcat(saveFolder, bvStr, nameStr, pngSuffix))
            saveas(ca, strcat(saveFolder, caStr, nameStr, pngSuffix))
            saveas(ba, strcat(saveFolder, baStr, nameStr, pngSuffix))
...
% Overall title and labels for subplot
figure(parentFig)
title(subplotFig,'Subject 11: Blocks 3 & 4','FontSize',titleFontSize)
xlabel(subplotFig,'Normalized Movement Duration','FontSize',labelFontSize);
ylabel(subplotFig,'Cart Velocity (m/s)','FontSize',labelFontSize);
