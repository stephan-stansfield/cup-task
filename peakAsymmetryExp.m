...
for blockNum = blockStart:blockEnd
    ...
    if plotTrial    
        ...
        figure('Name','Hand velocity','units','normalized','outerposition',[0 0 1 1]);
        cv = get(gcf,'Number');
        figTitle = ['Experimental Hand Velocity Profiles: Subject ', subjNum, ' Block ', block3or4 ];
    ...
    if plotTrial
        ...
        for trialNum = trialStart:1:trialEnd
            ...
            fileStr = strcat("experimental data trimmed/",subjStr,trialDate,trialStr,"/",num2str(trialNum), ".mat");
...
% significantly different from 1 at 5% signifiance level
[h5,p5,ci,stats] = ttest(cell2mat(medianBySubject(:,2)),1,'Tail','right');

[h1,p1,ci,stats] = ttest(cell2mat(medianBySubject(:,2)),1,'Tail','right',...
    'Alpha',.01);

manualT = (mean(cell2mat(medianBySubject(:,2))) - 1)/...
    std(cell2mat(medianBySubject(:,2)),1,'all','omitnan')/sqrt(10);

% Put total results for simulation type into table
overallResults(:,2) = {'Experimental Data';...
                        median(velRatio,'all','omitnan');...
                        mean(velRatio,'all','omitnan');...
                        std(velRatio,1,'all','omitnan');...
                        mean(cell2mat(medianBySubject(:,2)));...
                        std(cell2mat(medianBySubject(:,2)),1,'all','omitnan');...
                        h5; p5; h1; p1};
