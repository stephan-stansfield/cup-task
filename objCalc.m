...
%             disp('Limit: cart acceleration over limit')
    ...
    % Reassign variable names to capture any nullified solutions
    pos_sim     = output(:,1);
    theta_sim   = output(:,2);
    vel_sim     = output(:,3);
    omega_sim   = output(:,4);
    ...
    lensim = length(pos_sim);                                               % Length of simulated motion
    lenexp = length(pos);                                                   % Length of experimental motion

    if lensim ~= lenexp
        smaller = min(lensim,lenexp);

        % Shorten the longer arrays to agree dimensionally
        if lensim > lenexp                                                  % If simulated profiles are longer
            ...
            acc_RMSE         = acc;
            alpha_RMSE       = alpha;
        else                                                                % If experimental profiles are longer
    ...
    elseif objective == "features"

        % DEBUG
        
        val = featuresRMSE(acc_RMSE,acc_sim_RMSE,vel_RMSE, vel_sim_RMSE,theta_RMSE, theta_sim_RMSE,blockStr,blockNum,subjNum,num);

    end

    % If number of output arguments is greater than 1 and using a
    % derivative-based algorithm, need to define the gradient.
    % For the RMS error, the gradient for each variable should just be
    % the partial derivative of the square root function with respect
    % to that variable.
    if (nargout > 1)
        % Example problem gradient
%         gradient = [0, 0.5 / val];
    end 
