...
%     velChange_peaks = find(abs(diff(sign(acc)))==2)+1;                      % When sign changes between + and -
%     velChange_zeros = find(abs(diff(sign(acc)))==1)+1;                      % When sign changes to or from 0
    ...
    % DEBUG: remove semicolons to print number of peaks
    numPeaks = length(velChange);
